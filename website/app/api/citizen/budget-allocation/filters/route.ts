...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: aipRows, error: aipsError } = await client
      .from("aips")
      .select("fiscal_year,city_id,barangay_id")
      .eq("status", "published")
    ...
    const cityNameById = new Map(
      ((citiesResult.data ?? []) as ScopeNameRow[]).map((row) => [row.id, row.name?.trim() ?? ""])
    );
    ...
    const fallbackLgu = allLgusSorted[0] ?? null;
    const fallbackYear = allYearsSorted[0] ?? null;
