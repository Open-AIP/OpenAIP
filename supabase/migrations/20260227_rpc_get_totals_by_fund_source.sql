...
drop function if exists public.get_totals_by_fund_source(
  int,
  uuid
);

create or replace function public.get_totals_by_fund_source(
...
revoke all on function public.get_totals_by_fund_source(
  int,
  uuid
) from public;
...
grant execute on function public.get_totals_by_fund_source(
  int,
  uuid
) to authenticated;
